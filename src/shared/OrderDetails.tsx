...
// import { orderDetails } from "../helpers/functions/constants";
        ...
        <div className=" h-[450px] overflow-y-scroll custom-scrollbar flex flex-col gap-2">
          <div className="p-4 bg-base-gray-200">
            <div className="flex justify-between">
              <div className="">
                <p className={`${boldFont}`}>Order no. {orderDetails?.orderSummary?.checkOutOrderNumber}</p>
                <p className={`${boldFont}`}>{orderDetails?.orderSummary?.quantity} {orderDetails?.orderSummary?.quantity === 1 ? 'Item' : 'Items'}</p>
                <p className="text-sm">Order Date : {formatDate(orderDetails?.orderSummary?.transactionDate ?? '')}</p>
                <p className="text-sm">ETA : N/A</p>
              </div>
              <div>
                <p className={`${boldFont} !text-2xl`}>
                  Total: {formatMoney(orderDetails?.orderSummary?.checkoutTotal ?? 0)}
                </p>
              </div>
            </div>
            <div className="mt-2">
              <p className={`${boldFont} text-sm`}>Status</p>
            </div>
          </div>
          <div className="flex gap-2">
            <div className="w-1/2">
              <div className="flex items-center p-3 font-bold bg-gray-primary-400">
                <p>{orderDetails?.orderDetails.length === 1 ? 'Item' : 'items'}</p>
              </div>
            ...
            <div className="w-1/2">
              ...
              <div className="flex flex-col gap-4 p-4 border bg-base-gray-200">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm">Items total: {formatMoney(orderDetails?.paymentDetails?.itemTotal ?? 0)}</p>
                  <p className="text-sm">Delivery Fees: {formatMoney(orderDetails?.paymentDetails?.deliveryFees ?? 0)}</p>
                  <p className="text-sm">Total: {formatMoney(orderDetails?.paymentDetails?.total ?? 0)}</p>
                </div>
              </div>
              <div className="flex items-center p-3 font-bold bg-gray-primary-400">
              ...
              <div className="flex flex-col gap-4 p-4 border bg-base-gray-200">
                <div className="flex flex-col gap-1">
                  <p className="text-sm">{orderDetails?.deliveryInformation.destination}</p>
                  <p className="text-sm">
                    {orderDetails?.deliveryInformation.address}, {orderDetails?.deliveryInformation.region}
                  </p>
                </div>
              </div>
            </div>
